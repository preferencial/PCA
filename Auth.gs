/**
 * Auth.gs — Adaptador de autenticacao do Preferencial - PCA.
 *
...
function Auth_service_() {
  return AuthStandardService.configure({
    sessionKey: 'PREFERENCIAL___PCA_AUTH_SESSION',
...
function loginWithToken(username, password) {
  try {
    try {
      var normalizedUsername = String(username == null ? '' : username).trim();
      var normalizedPassword = String(password == null ? '' : password);
      if (!normalizedUsername || !normalizedPassword) {
        return { success: false, message: 'Informe usuario e senha.' };
      }
      var result = Auth_service_().login(normalizedUsername, normalizedPassword);
...
function isAuthenticatedByToken(tok) {
  try {
    try {
      try {
        var token = String(tok == null ? '' : tok).trim();
        ...
        for (var i = 1; i < data.length; i++) {
          if (String(data[i][0] == null ? '' : data[i][0]).trim() === token) {
            var expiresAt = Number(data[i][4]);
            if (!isFinite(expiresAt) || expiresAt <= now || !String(data[i][1] == null ? '' : data[i][1]).trim() || !String(data[i][2] == null ? '' : data[i][2]).trim()) {
              sheet.deleteRow(i + 1);
              return false;
            }
            return true;
      ...
      try {
        ...
        for (var i = 1; i < data.length; i++) {
          if (String(data[i][0] == null ? '' : data[i][0]).trim() === token) {
            ...
            var userId = String(data[i][1] == null ? '' : data[i][1]).trim();
            var username = String(data[i][2] == null ? '' : data[i][2]).trim();
            if (!userId || !username) {
              sheet.deleteRow(i + 1);
              return null;
            }
            return { userId: userId, username: username, role: data[i][3] || 'USER' };
          }
        }
        return null;
      } catch (error) {
