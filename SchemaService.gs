...
var SYNTHETIC_USERS_LOGIN_SCHEMAS = [
  { sheetName: 'Usuarios', headers: ['ID','Username','Password','Role','Nome','Email','CriadoEm'] }
];
  ...
  function getSpreadsheet_(options) {
    try {
      try {
        ...
        var idFromProperties = props.getProperty('SPREADSHEET_ID');
        if (idFromProperties) return SpreadsheetApp.openById(idFromProperties);
        // PCA guarda o ID do banco em DATABASE_ID e expoe getDb() (resolve/cria a planilha).
        ...
        // Tenta resolver via getBoundSpreadsheet_ (Script Properties / CONFIG).
        try { return getBoundSpreadsheet_(); } catch (_) {}
        // Ultimo recurso (web app standalone sem nada configurado): auto-provisiona e persiste.
        var created = SpreadsheetApp.create('Preferencial - PCA - Database');
        props.setProperty('DATABASE_ID', created.getId());
        return created;
      } catch (error) {
